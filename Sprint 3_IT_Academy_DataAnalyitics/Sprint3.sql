...
alter table transaction add constraint fk_cc
foreign key (credit_card_id) references credit_card(id);


/*Ejercicio 2
El departament de Recursos Humans ha identificat un error en el número de compte de l'usuari amb 
ID CcU-2938. Es requereix actualitzar la informació ingressada en l'IBAN. 
...
-- comprobamos cómo ha quedado la tabla al haber eliminado la columna pan 
SELECT *
FROM credit_card;
...
Elimina de la taula transaction el registre amb ID 02C6201E-D90A-1859-B4EE-88D2986D3B02 de la 
base de dades. */

DELETE from transaction 
WHERE id = '02C6201E-D90A-1859-B4EE-88D2986D3B02';

-- Comprobamos que ya no existe registro con ese id en la tabla
...
Nom de la companyia. Telèfon de contacte. País de residència. Mitjana de compra realitzat per 
...
SELECT company_name, phone, country, AVG(amount) AS purchase_average
FROM company
...
-- Revisamos el resultado de la vista creada
...
-- comprobamos el aspecto de la tabla
SELECT * 
FROM user;

-- Renombramos la columna email de user por personal_email

alter table user 
rename column email to personal_email;

-- Creamos un índice en el campo credit_card_id de la tabla transaction
...
-- Añadimos la columna fecha_actual a la tabla credit_card y establecemos el tipo de dato DATE
...
Assegura't d'incloure informació rellevant de totes dues taules i utilitza àlies per a canviar 
de nom columnes segons sigui necessari.
Mostra els resultats de la vista, ordena els resultats de manera descendent en funció de la 
variable ID de transaction. */
